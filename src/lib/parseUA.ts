...
export function parseUA(ua: string): ParsedUA {
  ...
  let os = "Otro", osDetail = "Otro";

  if (/iPhone|iPod/.test(ua)) {
    const v = ua.match(/OS (\d+[_\d]*)/);
    const ver = v ? v[1].replace(/_/g, ".") : "";
    os = "iOS"; osDetail = `iOS ${ver}`.trim();
  } else if (/iPad/.test(ua)) {
    const v = ua.match(/OS (\d+[_\d]*)/);
    const ver = v ? v[1].replace(/_/g, ".") : "";
    os = "iPadOS"; osDetail = `iPadOS ${ver}`.trim();
  } else if (/Android/.test(ua)) {
    const v = ua.match(/Android (\d+\.?\d*)/);
    const ver = v ? v[1] : "";
    os = "Android"; osDetail = `Android ${ver}`.trim();
  ...
  return { tipo, os, osDetail, browser, dispositivo: `${tipo} · ${osDetail} · ${browser}` };
}
